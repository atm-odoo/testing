...
class WooInstanceEpt(models.Model):
    ...
    def get_compare_data(self, record):
        ...
        :return: datatype > 'postivie' or 'nagative', comparison value
        """
        data_type = False
    ...
    def get_total_orders(self, record):
        ...
        def orders_of_current_week(record):
            self._cr.execute("""select id from sale_order where date(date_order) >=
                                ...
                                and woo_instance_id= %s and state in ('sale','done') order by date(date_order)
                        """ % record.id)
            return self._cr.dictfetchall()

        def orders_of_current_month(record):
            self._cr.execute("""select id from sale_order where date(date_order) >=
                                (select date_trunc('month', date(current_date))) and 
    ...
    def get_shipped_orders(self, record):
        ...
        shipped_query = """select so.id from stock_picking sp
                             ...
                             so.woo_instance_id=%s""" % record.id
    ...
    def get_total_products(self, record):
        ...
        self._cr.execute("""select count(id) as total_count from woo_product_template_ept where
                        exported_in_woo = True and woo_instance_id = %s""" % record.id)
        ...
        action = record.prepare_action(view, [('exported_in_woo', '=', True), ('woo_instance_id', '=', record.id)])
    ...
    def get_customers(self, record):
        ...
        if result:
            for id in result:
                customer_ids.append(id.get('partner_id'))
    ...
    def get_refund(self, record):
        ...
        """
        refund_query = """select id from account_move where woo_instance_id=%s and move_type='out_refund'""" % record.id
