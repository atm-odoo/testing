...
class WooProcessImportExport(models.TransientModel):
    ...
    def prepare_data_and_import_stock(self):
        ...
        for sku_chunk in split_every(100, sku):
            ...
            for res_product in res_products:
                products_stock, dublicate_woo_product, log_lines = self.prepare_data_for_inventory_adjustment(
                    woo_products, res_product, dublicate_woo_product, products_stock, common_log_line_obj, model_id,
                    log_lines)
    ...
    def prepare_data_for_inventory_adjustment(self, woo_products, res_product, dublicate_woo_product, products_stock,
            ...
            if res_product.get('manage_stock') and res_product.get('stock_quantity'):
                if product.product_id.type == 'product':
                    ...
                    if product.product_id.id not in dublicate_woo_product:
                        _logger.info(
                            "\n Adding dict in Woo product list for inventory adjustment: %s for Woo "
                            "product variant ID: %s", stock_data, product.variant_id)
        ...
        return products_stock, dublicate_woo_product, log_lines
    ...
    def export_woo_coupons(self):
        ...
        if coupons_ids and self._context.get('process'):
            ...
            if woo_coupons:
                coupons_obj.export_coupons(self.woo_instance_id, woo_coupons, common_log_book_id, model_id)
                if not common_log_book_id.log_lines:
                    common_log_book_id.unlink()
